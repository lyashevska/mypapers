...
%DIF DEL paper-before.tex   Sun Jan 17 15:20:06 2016
%DIF ADD paper-after.tex    Thu Jun  9 16:19:38 2016
...
by species distribution models\DIFdelbegin \DIFdel{, also known as }\DIFdelend \DIFaddbegin \DIFadd{; in particular, by }\DIFaddend habitat suitability or environmental niche models \citep{guisan2005}. Such models are constructed using survey data available at a limited set of sampling locations and allow one to create predictive species distribution maps on the basis of environmental data which are usually available \DIFdelbegin \DIFdel{at }\DIFdelend \DIFaddbegin \DIFadd{for }\DIFaddend a much larger set of locations \citep{guisan2000}. The number of sampling locations is known to affect the accuracy of the species distribution models and maps \citep{stockwell2002, wisz2008}.  Knowledge of the trade-off function between the number of sampling locations and the accuracy of the predictions is usually not obtained a priori\DIFdelbegin \DIFdel{with the risk that the survey either does not }\DIFdelend \DIFaddbegin \DIFadd{. Neither does the survey }\DIFaddend provide the required accuracy \DIFaddbegin \DIFadd{when the number of sampling locations is too low }\DIFaddend or is unnecessarily costly \DIFaddbegin \DIFadd{when the number of sampling locations is too high }\DIFaddend \citep{caughlan2001,reynolds2011}.

\DIFdelbegin \DIFdel{Yet, the effect }\DIFdelend \DIFaddbegin \DIFadd{Several studies evaluated effects }\DIFaddend of sample size on the accuracy of species distribution models \DIFdelbegin \DIFdel{was evaluated by few \mbox{%DIFAUXCMD
\citep[see e.g.][]{stockwell2002,pearson2007,wisz2008,hanberry2012}
}%DIFAUXCMD
}\DIFdelend \DIFaddbegin \DIFadd{\mbox{%DIFAUXCMD
\citep{stockwell2002,pearson2007,wisz2008,hanberry2012}
}%DIFAUXCMD
}\DIFaddend . For example, \citet{stockwell2002} assessed sample size requirements for modelling bird species in Mexico by random sampling \DIFaddbegin \DIFadd{between }\DIFaddend 1 \DIFdelbegin \DIFdel{to }\DIFdelend \DIFaddbegin \DIFadd{and }\DIFaddend 100 locations. \citet{wisz2008} considered three sample sizes (10, 30, and 100 locations) to evaluate the quality of model predictions using data for 46 species obtained from natural history collections. Finally, \citet{hanberry2012} used sample sizes ranging from 30 to 2500 locations to model tree species in northeasten  Minnesota. All these studies consider presence--absence maps, but often, predictive species abundance maps in the form of numerical or biomass density are to be preferred,  because they are more informative than presence--absence maps \citep{vieira2012, cozzi2013}.  \citet{fortin1989} constructed such maps using sugar-maple tree density data gathered in  southwestern Qu\'{e}bec.  The authors evaluated the ability to predict spatial patterns using different sample sizes and designs. They considered two sample sizes of 50 and 64 points, both derived from a 200-point dataset.

Using real datasets only, as \citet{fortin1989} did, \DIFdelbegin \DIFdel{the possibilities of studying the effect of the type of sampling design and sample size are limited. This limitation was }\DIFdelend \DIFaddbegin \DIFadd{limits comparison between the effects of different sample designs and sample number sizes as well as uncertainties in the model's parameter values. These limitations were }\DIFaddend recognised in recent studies, such as those  by \citet{perner2004, rachowicz2006, bijleveld2012} and \citet{foster2014}. 
\DIFdelbegin \DIFdel{These studies follow an approach whereby first }\DIFdelend \DIFaddbegin \DIFadd{An updated approach is to first simulate }\DIFaddend a spatial field resembling reality as much as possible, a peseudo-reality, \DIFdelbegin \DIFdel{is simulated, which is }\DIFdelend \DIFaddbegin \DIFadd{which is then }\DIFaddend subsequently sampled using different sampling designs. The performance of sampling designs is then compared by confronting predictions with simulated values that serve as ground-truth.  \citet{zurell2010} call this the virtual ecologist approach.  Following this approach, \citet{bijleveld2012} \DIFdelbegin \DIFdel{, for example, }\DIFdelend used the results of an existing intertidal benthic monitoring programme to construct various spatial models with an exponential spatial autocorrelation function. With these models they simulated virtual populations with a Normal distribution and sampled these populations using different sampling designs. They provided a trade-off function between sampling distance and prediction error which was rather flat for those virtual species that hardly showed spatial autocorrelation, but much steeper for species with strong spatial autocorrelation. The \DIFdelbegin \DIFdel{authors }\DIFdelend assumed normality of the data \DIFdelbegin \DIFdel{which }\DIFdelend was clearly violated by the empirical data \DIFdelbegin \DIFdel{if only }\DIFdelend because of the many zero observations.

The assumption of normality is a common practice, \DIFdelbegin \DIFdel{but when we are }\DIFdelend \DIFaddbegin \DIFadd{because when }\DIFaddend dealing with species abundance (count) data \DIFdelbegin \DIFdel{, even }\DIFdelend the more obvious Poisson distribution is rarely applicable. Ecological count data have two properties that ask for a specific treatment, other than relying on the classical assumption of independent and normally distributed data. These properties are zero-inflation \citep{martin2005, clarke1988, lewis2011} and spatial autocorrelation, i.e. nearby observations are more similar than observations far apart, even when environmental conditions do not differ. Hitherto most studies have dealt with these two properties, but only one at a time \citep{tyre2003, bijleveld2012}.
...
Contrary to \citet{tyre2003}, the \citeauthor{bijleveld2012} study took account of the autocorrelation by using a stochastic model that included spatial autocorrelation of the error. But, as \DIFdelbegin \DIFdel{we }\DIFdelend mentioned earlier, they simulated normally distributed data. Clearly, there is a need to integrate both properties in a single study and to examine how zero-inflation and autocorrelation may affect recommendations \DIFdelbegin \DIFdel{concerning }\DIFdelend \DIFaddbegin \DIFadd{for }\DIFaddend the optimal sampling design, sample size, and distance between samples.

\DIFdelbegin \DIFdel{Only few studies }\DIFdelend \DIFaddbegin \DIFadd{Studies that }\DIFaddend simultaneously address zero-inflation and autocorrelation for species abundances \citep[see e.g.][]{recta2012, boyd2015} \DIFdelbegin \DIFdel{, but none of them address }\DIFdelend \DIFaddbegin \DIFadd{do not treat }\DIFaddend the question of optimal sampling design.  We \DIFaddbegin \DIFadd{attempt to }\DIFaddend fill this gap by following a paper by \citet{lyashevska2016a} in simulating fields with zero-inflated, spatially autocorrelated count data, and sampling the fields repeatedly with different sampling designs. More specifically, we will sample the fields by grid-sampling with a varying spacing. The \DIFdelbegin \DIFdel{objective }\DIFdelend \DIFaddbegin \DIFadd{aim }\DIFaddend of this paper is to quantify the trade-off between \DIFdelbegin \DIFdel{the }\DIFdelend grid-spacing and \DIFdelbegin \DIFdel{the }\DIFdelend accuracy of predictions of species-abundance model parameters on a fine grid for mapping, for species that show zero-inflation and spatial autocorrelation. Most species will show these two properties \DIFaddbegin \DIFadd{\mbox{%DIFAUXCMD
\citep[see][and references therein]{martin2005}
}%DIFAUXCMD
}\DIFaddend .
...
\begin{equation}
    P(Y_i=y \DIFaddbegin \DIFadd{| \eta_{i}}\DIFaddend )=
\begin{cases}
\pi_i+(1-\pi_i)\text{exp}(-\mu_i) & y=0 \\
(1-\pi_i)\frac{\text{exp}(-\mu_i)\mu_i^{y}}{y!}& y=1,2,3,\ldots
\end{cases}
\end{equation}
where $Y_i$ is the count at location $i$, \DIFaddbegin \DIFadd{$\eta_{i}$ is spatially dependent random effect, }\DIFaddend $\pi_i$ the probability of a Bernoulli zero at location $i$, and $1-\pi_i$ is the probability of a Poisson count, either zero or non-zero. The intensity (mean number of individuals) of the Poisson process at location $i$ is $\mu_i$. The first part of the model is the overall probability of zero \citep{hilbe2007}.
...
Following \citet{diggle2007}, \DIFdelbegin \DIFdel{hereafter }\DIFdelend the sum of the trend and residual, representing the transformed model parameter, is referred to as the signal $S$, for instance $S_{\mathrm{B},i}= \mathbf{x}_{\mathrm{B},i}^{T}\boldsymbol{\beta}_{\rm{B}} + \eta_{\mathrm{B},i}$. For convenience, all the parameters in one model, including the type of correlation function, are collected in  a vector: $\boldsymbol{\theta}_{\mathrm{B}}=(\boldsymbol{\beta}_{\mathrm{B}}, \phi_{\mathrm{B}}, \tau_{\mathrm{B}}^2, \sigma_{\mathrm{B}}^2, \rho_{\mathrm{B}})$ and $\boldsymbol{\theta}_{\mathrm{P}}=(\boldsymbol{\beta}_{\mathrm{P}}, \phi_{\mathrm{P}}, \tau_{\mathrm{P}}^2, \sigma_{\mathrm{P}}^2, \rho_{\mathrm{P}})$. To avoid confusion the model parameters $\boldsymbol{\theta}_{\mathrm{B}}$ and $\boldsymbol{\theta}_{\mathrm{P}}$ are referred to as hyperparameters; with model parameters we mean the parameters $\pi$ and $\mu$.

The aim of \DIFaddbegin \DIFadd{evaluating }\DIFaddend the sampling strategy \DIFdelbegin \DIFdel{to be evaluated }\DIFdelend is to map the prevalence parameter $\pi$ of the Bernoulli distribution and the intensity parameter $\mu$ of the Poisson distribution. \DIFdelbegin \DIFdel{So }\DIFdelend \DIFaddbegin \DIFadd{Please }\DIFaddend note that the \DIFdelbegin \DIFdel{aim }\DIFdelend \DIFaddbegin \DIFadd{objective }\DIFaddend is not to predict the species abundance counts, but \DIFaddbegin \DIFadd{to use the observed counts in the sample to estimate, at the desired sites $i$, the }\emph{\DIFadd{expected}} \DIFadd{counts conditional on the values $\bf{x}_i$ of the covariates and }\DIFaddend the \DIFdelbegin \emph{\DIFdel{expected}} %DIFAUXCMD
\DIFdel{counts , conditional on covariates that define a spatial trend and the observed counts at the sample to be evaluated}\DIFdelend \DIFaddbegin \DIFadd{random effects $\eta_i$ that express spatial dependence}\DIFaddend .
We believe that predicting the counts themselves is not feasible in our situation, and \DIFdelbegin \DIFdel{besides }\DIFdelend not of practical relevance.

\DIFdelbegin \DIFdel{Broadly speaking, our }\DIFdelend \DIFaddbegin \DIFadd{Our }\DIFaddend evaluation procedure is as follows. The SIBES data are used to \DIFdelbegin \DIFdel{calibrate }\DIFdelend \DIFaddbegin \DIFadd{estimate the parameters of }\DIFaddend a ZIP model. Several steps are involved in \DIFdelbegin \DIFdel{calibration}\DIFdelend \DIFaddbegin \DIFadd{estimation}\DIFaddend . First, a ZIP model is fitted by maximum likelihood assuming that both residuals $\eta_{\mathrm{B},i}$ and $\eta_{\mathrm{P},i}$ are spatially independent. The fitted model parameters are then used to classify a zero count either as a Bernoulli or a Poisson zero, and to construct two datasets: the Bernoulli dataset with zeros (absent) and ones (present), and the Poisson subset, containing the SIBES locations with a one in the Bernoulli dataset, with counts. In the next step these two data sets are used to fit the two submodels for the parameters $\pi_i$ and $\mu_i$, but now accounting for spatial autocorrelation. This is done by simulating a large sample of signals $S_{\mathrm{B}}$ and $S_{\mathrm{P}}$ at the SIBES locations by Markov chain Monte Carlo (MCMC) using initial estimates of the hyperparameters, followed by Monte Carlo Maximum Likelihood (MCML) estimation of the hyperparameters. \DIFaddbegin \DIFadd{The final MCML estimates of the hyperparameters are used to simulate signals $S_{\mathrm{B}}$ and $S_{\mathrm{P}}$ at the SIBES locations conditional on the observations at these locations.
}\DIFaddend 

The fitted hyperparameters are then used to \DIFdelbegin \DIFdel{predict }\DIFdelend \DIFaddbegin \DIFadd{simulate }\DIFaddend the Bernoulli signal ($S_{\mathrm{B}}$) and Poisson signal ($S_{\mathrm{P}}$) at the nodes of a very fine square grid with a spacing of 100 m covering the study area. This \DIFdelbegin \DIFdel{prediction }\DIFdelend grid is extended with 1000 randomly selected validation locations in between the grid-points. 
\DIFdelbegin \DIFdel{The prediction method is simple kriging with an external drift, using silt, the squared values of silt, and altitude as external drift variables (predictors).
}\DIFdelend \DIFaddbegin 

%DIF > The method is sequential Gaussian simulation conditional on the simulated signals $S_{\mathrm{B}}$ and $S_{\mathrm{P}}$ at the SIBES locations.
\DIFaddend 

In the next step the \DIFdelbegin \DIFdel{predicted }\DIFdelend \DIFaddbegin \DIFadd{simulated }\DIFaddend signals at the grid-nodes and validation points are used to simulate fields with count data. One field with statistics closest to those of the SIBES data is selected, and \DIFaddbegin \DIFadd{underlying $S_{\mathrm{B}}$ and $S_{\mathrm{P}}$ fields are }\DIFaddend repeatedly sampled by grid-sampling. A range of grid-spacings is applied. Each selected grid is used to predict the model parameters $\pi_i$ and $\mu_i$ at the validation locations.  By comparing these predictions with the true \DIFaddbegin \DIFadd{(original) }\DIFaddend model parameters at the validation points the quality of the predictions is assessed. More details on all steps but the first \DIFdelbegin \DIFdel{two (calibration }\DIFdelend \DIFaddbegin \DIFadd{(estimation }\DIFaddend of ZIP model \DIFdelbegin \DIFdel{and prediction of signals}\DIFdelend \DIFaddbegin \DIFadd{parameters}\DIFaddend ) are given below. For details on the first \DIFdelbegin \DIFdel{two steps }\DIFdelend \DIFaddbegin \DIFadd{step }\DIFaddend we refer to \citet{lyashevska2016a}.

\begin{enumerate}
\item
\DIFdelbegin \DIFdel{After prediction of the signal }\DIFdelend \DIFaddbegin 

\DIFadd{The simulated signals }\DIFaddend $S_{\mathrm{B}}$ at the nodes of \DIFdelbegin \DIFdel{the }\DIFdelend 100 m grid \DIFdelbegin \DIFdel{, 100 fields (square grids with spacing of 100 m) with spatially unstructured noise (white noise) are simulated from a Normal distribution with a variance equal to $\hat{\tau}^2_{\mathrm{B}}$. The predicted signal ($S_{\mathrm{B}}$) is added to the simulated fields with white noise.The resulting fields are backtransformed by second-order Taylor expansion }\DIFdelend \DIFaddbegin \DIFadd{extended with 1000 validation locations are backtransformed using the inverse of the link function in Eq.~\ref{eq:glsm} }\DIFaddend to give 100 \DIFdelbegin \DIFdel{simulated }\DIFdelend fields of the prevalence parameter $\pi$ of the Bernoulli distribution. The same procedure is followed for the $S_{\mathrm{P}}$ to give 100 \DIFdelbegin \DIFdel{simulated }\DIFdelend fields of the intensity parameter $\mu$ of the Poisson distribution.

\item
\DIFdelbegin \DIFdel{Each simulated field of $\pi$ is used to simulate one field with presence/absence indicators, giving 100 fields with indicators. Besides, each simulated field of $\mu$ is used to simulate one field with Poisson counts, giving 100 fields with counts. These two sets of 100 fields are multiplied pair-by-pair, to give 100 fields with zero-inflated counts.
}\DIFdelend %DIF > Each field of $\pi$ is used to simulate one field with presence/absence indicators, giving 100 fields with indicators. Besides, each field of $\mu$ is used to simulate one field with Poisson counts, giving 100 fields with counts. These two sets of 100 fields are multiplied pair-by-pair, to give 100 fields with zero-inflated counts.
%DIF > 
%DIF > \item
%DIF > From the 100 fields with zero-inflated counts, one field is selected with summary statistics closest to these summary statistics of the SIBES data. 
%DIF > We used as summary statistics the fraction of zeroes, the mean of the Poisson counts, and the variance of the Poisson counts. 
%DIF > This leads to three criteria (one for each summary statistic), namely the difference between value of summary statistic for the field and for the SIBES data.
%DIF > These criteria were standardized by dividing through their standard deviation. The weighted sum of the values was calculated, with weights equal to 0.5 (for fraction of zeroes), 0.25 and 0.25. 
...
\caption{Empirical species abundance map of \textit{Macoma balthica} \protect\subref{fig:AbundanceRaw} and unconditional intensity map \protect\subref{fig:field38} \DIFdelbeginFL \DIFdelFL{predicted }\DIFdelendFL \DIFaddbeginFL \DIFaddFL{conditionally simulated }\DIFaddendFL to the nodes of 100 m grid\DIFdelbeginFL \DIFdelFL{by simple kriging with an external drift. From \mbox{%DIFAUXCMD
\citet{lyashevska2016a}
}%DIFAUXCMD
}\DIFdelendFL .}
\label{fig:rawfield}
\end{figure}
...
\item
The \DIFdelbegin \DIFdel{selected field with zero-inflated counts is }\DIFdelend \DIFaddbegin \DIFadd{two underlying fields are }\DIFaddend sampled on a grid with a spacing of 200, 400, 800, 1600, and 3200 m. For each grid-spacing 100 samples are randomly selected. The corresponding number of grid-points was on average 28505, 7130, 1783, 446, and 110, respectively. 
An overlay is made of the selected grids \DIFdelbegin \DIFdel{and the selected field with simulated zero-inflated counts to obtain the `pseudo-observations' of counts that will be }\DIFdelend \DIFaddbegin \DIFadd{which are then }\DIFaddend used in prediction of the prevalence parameter $\pi$ and the intensity parameter $\mu$ at the validation locations.

\item
The model parameters $\pi$ and $\mu$ at the 1000 validation locations are predicted by the same method as used in simulating our pseudo-reality, being simple kriging with an external drift, using silt, silt-squared and altitude as external drift variables. Ideally, for each grid-sample the hyperparameters are estimated from the `pseudo-observations' of zero-inflated counts at the grid-points with Markov chain Monte Carlo maximum likelihood (MCML). \DIFaddbegin \DIFadd{Using these hyperparameters signals $S_{\mathrm{B}}$ and $S_{\mathrm{P}}$ should be simulated again conditional on the simulated counts (pseudo-observations).  }\DIFaddend However, this is not feasible due to the computing time involved. Therefore, in predicting from the selected grid-points to the validation points we used the hyperparameters that were also used to simulate all 100 fields. These hyperparameters (referred to hereafter as the parent-hyperparameters) were estimated by MCML from the SIBES data. In practice these hyperparameters are unknown, so that by using the unknown parent-hyperparameters we ignore the contribution of uncertainty about the hyperparameters to the uncertainty about the predictions.
...
\label{eq:mse}
\end{equation}
with $n$ the number of validation points ($n=1000$), $\hat{\pi_{i}}$ the predicted prevalence parameter at location $i$ and $\pi_{i}$ the `pseudo-observed' intensity parameter. For intensity parameter $\mu$ MSE is computed from the subset of validation points with a simulated \DIFaddbegin \DIFadd{value of }\DIFaddend 1 \DIFaddbegin \DIFadd{for the presence/absence indicator }\DIFaddend (species present). This subset contains 211 points. MSE was also calculated for the product of $\pi$ and $\mu$, representing unconditional intensity (intensity not conditioned on presence). For this product again all 1000 validation \DIFaddbegin \DIFadd{points }\DIFaddend are used.

For each grid-spacing we have 100 grid-samples. All 100 grid-samples are used in prediction with the fixed parent-hyperparameters $\boldsymbol{\theta}_{\mathrm{B}}$ and $\boldsymbol{\theta}_{\mathrm{P}}$ (no sample-specific estimation), leading to 100 estimates of MSE$(\pi)$, MSE$(\mu)$ and MSE$(\pi \cdot \mu)$. The distribution of these 100 estimates is an estimate of the sampling distribution of the estimated mean quality of model-based predictions of the model parameters $\pi$ and $\mu$. Only \DIFdelbegin \DIFdel{four }\DIFdelend \DIFaddbegin \DIFadd{five }\DIFaddend grid-samples are used in prediction with variable sample-specific estimates of the hyperparameters $\boldsymbol{\theta}_{\mathrm{B}}$ and $\boldsymbol{\theta}_{\mathrm{P}}$, so these \DIFdelbegin \DIFdel{four }\DIFdelend \DIFaddbegin \DIFadd{five }\DIFaddend estimates give a very rough estimate of the sampling distribution only.

\section{Results} \label{sec:results}

\subsection{Prevalence}
The mean of \DIFdelbegin \DIFdel{MSE }\DIFdelend \DIFaddbegin \DIFadd{the 100 MSEs (1 MSE per grid of a given spacing) }\DIFaddend of the predicted species prevalence parameter $\pi$ increased with increasing grid-spacing (Fig.~\ref{fig:mse.pi}).
Using the fixed parent-hyperparameters the increase was from \DIFdelbegin \DIFdel{a value close to 0 }\DIFdelend \DIFaddbegin \DIFadd{0.006 }\DIFaddend at 200 m to \DIFdelbegin \DIFdel{0.001 }\DIFdelend \DIFaddbegin \DIFadd{0.008 }\DIFaddend at 3200 m. \DIFdelbegin \DIFdel{Except for the largest spacing of 3200 m, the }\DIFdelend \DIFaddbegin \DIFadd{The }\DIFaddend variance of MSE between the 100 grid samples was small \DIFaddbegin \DIFadd{for all grid-spacings}\DIFaddend .

The mean of the \DIFdelbegin \DIFdel{four }\DIFdelend \DIFaddbegin \DIFadd{five }\DIFaddend MSE values using hyperparameters estimated from the grid-samples, was for all spacings larger than the mean of MSE using the fixed parent-hyperparameters, especially for the largest spacing \DIFaddbegin \DIFadd{of 3200 m}\DIFaddend . This shows that the contribution of uncertainty about the hyperparameters to the uncertainty about the predictions was substantial. Remarkable is the strong increase of the mean MSE beyond a spacing of 1600 m.
