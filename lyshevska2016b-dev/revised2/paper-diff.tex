...
%DIF DEL paper-before.tex   Thu Jun  9 16:19:38 2016
%DIF ADD paper-after.tex    Tue Aug 16 15:27:40 2016
...
by species distribution models; in particular, by habitat suitability or environmental niche models \citep{guisan2005}. Such models are constructed using survey data available at a limited set of sampling locations and allow one to create predictive species distribution maps on the basis of environmental data which are usually available for a much larger set of locations \citep{guisan2000}. The number of sampling locations is known to affect the accuracy of the species distribution models and maps \citep{stockwell2002, wisz2008}.  Knowledge of the trade-off function between \DIFdelbegin \DIFdel{the }\DIFdelend number of sampling locations and \DIFdelbegin \DIFdel{the }\DIFdelend accuracy of the predictions is usually not obtained a priori.  \DIFdelbegin \DIFdel{Neither does the survey provide the required accuracy when the number of sampling locations }\DIFdelend \DIFaddbegin \DIFadd{If number of samples }\DIFaddend is too low\DIFdelbegin \DIFdel{or is unnecessarily costly when the number of sampling locations }\DIFdelend \DIFaddbegin \DIFadd{, accuracy will suffer by an unknown ammount: if sampling intensity }\DIFaddend is too high\DIFaddbegin \DIFadd{, the design will be unnecessarily costly.  }\DIFaddend \citep{caughlan2001,reynolds2011}.
...
\DIFaddbeginFL \DIFaddFL{All 100 grid-samples were used in prediction with fixed parent-hyperparameters and only five grid-samples were used in prediction with variable sample-specific estimates.  }\DIFaddendFL The first grid-spacing (200 m) with estimated hyperparameters could not be estimated due to memory constraints.}
\label{fig:mse}
\end{figure}
